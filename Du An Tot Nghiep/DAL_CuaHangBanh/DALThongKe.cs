...
namespace DAL_CuaHangBanh
{
    public class DALThongKe
    {
        ...
        public class DAL_ThongKe
        {
            ...
            // -----------------------------
            // ✅ Thống kê tổng quát (đã sửa logic tính tổng doanh thu)
            // -----------------------------
            public DataTable GetThongKeTongQuat(DateTime? ngay = null)
            {
                string query = @"
                    ...
                    SELECT 
                        (SELECT SUM(TongTien) FROM DoanhThu) AS TongDoanhThu,
                        (SELECT SUM(CT.SoLuong)
                         FROM CT_HoaDon CT 
                         JOIN HoaDon HD ON CT.MaHoaDon = HD.MaHoaDon
                         WHERE (@Ngay IS NULL OR CAST(HD.DateCheck AS DATE) = @Ngay)) AS TongSanPham,
                        (SELECT TOP 1 TenSanPham FROM SPBanChay) AS SanPhamBanChayNhat;
                ";
            ...
            public DataTable GetTiLeSanPhamBanChay(DateTime? ngay)
            {
                string query = @"
